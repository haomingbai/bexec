...
struct when_all_state {
  ...
  void child_value() noexcept { finish_one(); }
  ...
  void finish_one() noexcept {
    ...
    if (final_terminal == terminal_kind::error) {
      bexec::set_error(std::move(*receiver_to_complete),
  ...
  void complete_empty() noexcept { bexec::set_value(std::move(receiver)); }
...
template <class Receiver, class ErrorVariant, class SenderTuple,
          std::size_t Index>
