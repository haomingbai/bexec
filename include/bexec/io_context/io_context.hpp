...
/**
 * @brief A simple thread-safe FIFO execution context.
 *
 * Despite the name, this type does not perform file, socket, or OS IO. It is a
 * small run-loop patterned after common io_context APIs.
 */
class io_context {
 public:
  class scheduler;
  ...
  /** @brief Returns a scheduler associated with this execution context. */
  ...
  bool enqueue(std::function<void()> work) { return post(std::move(work)); }
...
class io_context::scheduler {
 ...
 private:
  ...
  explicit scheduler(io_context& context) : context_(&context) {}
...
#include <bexec/io_context/detail/schedule_sender.hpp>
