...
using value_types_of_t =
    detail::gather_signatures_t<set_value_t, completion_signatures_of_t<Sender, Env>,
                                Tuple, Variant>;

template <class Sender, class Env = empty_env,
          template <class...> class Variant = variant_or_empty>
using error_types_of_t =
    detail::gather_signatures_t<set_error_t, completion_signatures_of_t<Sender, Env>,
                                bexec::single_type, Variant>;

template <class Sender, class Env = empty_env>
inline constexpr bool sends_stopped =
    (completion_signatures_of_t<Sender, Env>::template count_of<set_stopped_t>() != 0);
