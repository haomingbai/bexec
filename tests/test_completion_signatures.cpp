...
concept rvalue_startable = requires(Operation operation) {
    bexec::start(std::move(operation));
};

template <class Receiver>
concept lvalue_set_value_receiver = requires(Receiver receiver) {
    bexec::set_value(receiver);
};
...
void test_completion_signatures() {
    ...
    static_assert(std::same_as<
                  bexec::completion_signatures_of_t<just_int>,
                  bexec::completion_signatures<bexec::set_value_t(int)>>);
    static_assert(std::same_as<
                  bexec::value_types_of_t<just_int>,
                  std::variant<std::tuple<int>>>);
    static_assert(std::same_as<
                  bexec::error_types_of_t<just_int>,
                  bexec::type_list<>>);
